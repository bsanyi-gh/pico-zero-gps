...
static inline int16_t tableY() { return 55; }
static inline int16_t tableWidth() { return (tft.width() * 45) / 100; }
static inline int16_t tableHeight() { return tft.height() - tableY() - 40; }

static inline int16_t circleAreaX() { return tableX() + tableWidth() + 8; }
static inline int16_t circleAreaY() { return tableY(); }
static inline int16_t circleWidth() { return tft.width() - circleAreaX() - 5; }
static inline int16_t circleHeight() { return tft.height() - circleAreaY() - 40; }
...
void ScreenSats::drawContent() {
    if (firstDraw || sortOrderChanged) {
        ...
        uint8_t satCount = c1_sharedGpsData.satelliteCount;
...
void ScreenSats::layoutComponents() {
    // Back gomb jobb alsó sarokban — igazodva a többi képernyőhöz
...
void ScreenSats::drawTitle() {
    ...
    tft.drawString("GPS Satellites", tft.width() / 2, 20);
    tft.setFreeFont();
}
...
void ScreenSats::drawSatelliteTable() {
    ...
    tft.fillRect(tableX(), tableY(), tableWidth(), 24, TFT_BLACK);
    ...
    tft.drawString(valueBuffer, tableX(), tableY() + 12);

    // Táblázat fejléc
    int16_t headerTextY = tableY() + 28;
    if (firstDraw || sortOrderChanged) {
        // A fejléc területének törlése
        tft.fillRect(tableX(), headerTextY, tableWidth(), 12, TFT_BLACK);
        uint16_t prnColor = (currentSortType == SatelliteDb::BY_PRN) ? TFT_CYAN : TFT_YELLOW;
        tft.setTextColor(prnColor, TFT_BLACK);
        tft.drawString("PRN", tableX(), headerTextY);

        tft.setTextColor(TFT_YELLOW, TFT_BLACK);
        tft.drawString("   Elv   Azm ", tableX() + 18, headerTextY);

        uint16_t snrColor = (currentSortType == SatelliteDb::BY_SNR) ? TFT_CYAN : TFT_YELLOW;
        tft.setTextColor(snrColor, TFT_BLACK);

        // SNR pozíció arányosítva a tábla szélességéhez
        tft.drawString("SNR", tableX() + (tableWidth() * 75) / 100, headerTextY);

        tft.drawFastHLine(tableX(), headerTextY + 10, tableWidth(), TFT_DARKGREY);
    }

    int16_t currentY = headerTextY + 22;
    ...
    for (const auto &sat : satellites) {
        ...
        tft.fillRect(tableX(), currentY, tableWidth(), lineHeight, TFT_BLACK);
    ...
    if (itemCount < MAX_SATS_TABLE_ITEMS) {
        tft.fillRect(tableX(), currentY, tableWidth(), (MAX_SATS_TABLE_ITEMS - itemCount) * lineHeight, TFT_BLACK);
    ...
    if (satellites.size() > MAX_SATS_TABLE_ITEMS) {
        ...
        tft.drawString(msg, tableX(), currentY);
    } else {
        // töröljük a felirat helyét
        tft.fillRect(tableX(), currentY, tableWidth(), 28, TFT_BLACK);
    }
...
void ScreenSats::drawSatelliteCircle() {
    // Dinamikusan számolt kör terület és centrum
    const int16_t cAreaX = circleAreaX();
    const int16_t cAreaY = circleAreaY();
    const int16_t cWidth = circleWidth();
    const int16_t cHeight = circleHeight();

    // Háttér törlése
    tft.fillRect(cAreaX, cAreaY, cWidth, cHeight, TFT_BLACK);

    const int16_t centerX = cAreaX + cWidth / 2;
    const int16_t centerY = cAreaY + cHeight / 2;
    int16_t maxRadius = std::min<int16_t>(cWidth, cHeight) / 2 - 8;
    if (maxRadius < 10)
        maxRadius = 10;
...
bool ScreenSats::handleTouch(const TouchEvent &event) {
    ...
    if (!event.pressed) {
        ...
        // PRN touch?
        int16_t prnRight = tableX() + std::min<int16_t>(40, tableWidth() / 3);
        int16_t headerYTop = tableY() + 25;
        int16_t headerYBottom = tableY() + 45;
        if (x >= tableX() && x <= prnRight && y >= headerYTop && y <= headerYBottom) {
            handlePrnHeaderClick();
            return true;

        } // SNR touch? (jobb oldali 75% pozíció környéke)
        else if (x >= tableX() + (tableWidth() * 70) / 100 && x <= tableX() + tableWidth() && y >= headerYTop && y <= headerYBottom) {
