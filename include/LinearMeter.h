...
void verticalLinearMeter(TFT_eSprite *sprite, int meterHeight, int meterWidth, const char *category, float val, float minVal, float maxVal, int x, int y, int w, int h, int g, int n, byte s, boolean mirrored = false) {
    ...
    auto bgColorForGlobalY = [&](int16_t globalY) -> uint16_t { return sprite->color565(0, 10 + (globalY * 22) / DISPLAY_H, 18 + (globalY * 34) / DISPLAY_H); };

    // Sprite háttér: azonos gradiens/rács, mint a fő képernyőn.
    for (int16_t localY = 0; localY < meterHeight; localY++) {
        const int16_t globalY = drawY + localY;
        const uint16_t bg = bgColorForGlobalY(globalY);
        sprite->drawFastHLine(0, localY, meterWidth, bg);
    }
    for (int16_t globalX = 0; globalX < DISPLAY_W; globalX += 20) {
        const int16_t localX = globalX - drawX;
        if (localX >= 0 && localX < meterWidth) {
            sprite->drawFastVLine(localX, 0, meterHeight, sprite->color565(0, 18, 26));
        }
    }
    ...
    sprite->setTextColor(TFT_YELLOW, bgColorForGlobalY(drawY + titleY));
    sprite->setTextDatum(mirrored ? TR_DATUM : TL_DATUM);
    sprite->drawString(category, mirrored ? meterWidth : 0, titleY, 2); // TFT_eSPI Font2
    ...
    for (int b = 1; b <= n; b++) {
        ...
        if (buf[0] != '\0') {
            sprite->setTextSize(1);
            sprite->setTextColor(TFT_WHITE, bgColorForGlobalY(drawY + barY));
            sprite->setTextDatum(mirrored ? TR_DATUM : TL_DATUM);
